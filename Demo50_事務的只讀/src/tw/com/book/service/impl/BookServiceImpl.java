...
public class BookServiceImpl implements IBookService {
	...
	* 
	* readOnly:指定當前的一系列操作是否為只讀
	* 	在加鎖的情況下，自己對那條數據的操作，其他用戶不能進行任何操作
	* 
	* 	事務中不管有無寫的操作;若設置為只讀，mysql會在請求訪問資料庫時，不加鎖提高性能
	* 	但事務中有寫的操作;若設置為只讀，因為不加鎖，所以會產生髒讀、不可重複讀、幻讀的狀況
	* 
	* rollbackFor|rollbackForClassName|noRollbackFor|noRollbackForClassName
	*/
	
	@Transactional(readOnly = true)// 為了讓扣庫存、扣餘額視為同一個事務，所以在這個方法上加上註解
	@Override
